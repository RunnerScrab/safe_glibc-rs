...
fn safely_get_fnaddr_from_got<FnType>(needle: &[u8], offset_from_needle: usize) -> Option<FnType> {
    let pfun = safely_get_fnaddrval_from_got(needle, offset_from_needle);
    if pfun != 0 {
        let safe_fn = transmute::<usize, FnType>(pfun);
        return Some(safe_fn);
    } else {
...
fn main() {
    ...
    fn phdrinfo_callback(pinfo: usize, size: usize, pdata: usize) -> i32 {
        println!(
            "phdrinfo_callback: 0x{:x}, 0x{:x}, 0x{:x}",
            pinfo, size, pdata
        );
        ...
        for idx in 0..size {
            print!(
                "0x{:x}{}",
                transmute::<usize, &[u8; u32::MAX as usize]>(buffer)[idx],
                if idx < size - 1 { "," } else { "\n" }
            );
        }
        let rname = transmute::<usize, &usize>(buffer + 8);
        let pname = transmute::<usize, &[u8; u32::MAX as usize]>(*rname);
        let mut idx: usize = 0;
